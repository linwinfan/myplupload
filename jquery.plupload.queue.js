							...
							'<div class="plupload_filelist_footer">' +
								'<div class="plupload_file_name">' +
									'<div class="plupload_buttons">' +
										'<a href="#" class="plupload_button plupload_add" id="' + id + '_browse">' + _('浏览') + '</a>' +
	...
	$.fn.pluploadQueue = function(settings) {
		if (settings) {
			...
			this.each(function() {
				...
				//}
				settings.url=plupload.buildUrl(settings.url,settings.load_params);
...
/*uploader.bind('StateChanged', function(up,file,props) {
	...
	$.fn.uploadRender = function(resps,renderContainer,up){
		...
		$(resps).each(function(){
			...
			var isFlow = refType.indexOf("form_") >= 0;//是否是工作流上传的附件 
			
			if(this.deleteByName == null){
				html.push("<li class='plupload-row' id='"+_id+id+"_r' title='"+name+"'>");
				if(isFlow)
					html.push("<span class='upload-filename'><font color='blue'>"+name+"</font>  上传时间:<font color='blue'>"+this.createdDate+"</font>  上传步骤:<font color='blue'>"+this.step_+"</font>  上传人:<font color='blue'>"+this.createdByName+"</font></span>");
				else
		...
		$(resps).each(function(){
			...
			if(this.deleteByName != null && isFlow){
				if(index == 0){
					html.push("<li>--------------- 附件删除列表 ---------------</li>");
				}
				
				html.push("<li class='plupload-row'>");
				html.push("<span class='upload-filename'><font color='blue'>"+this.name+"</font></span>");
				html.push("<span class='upload-filename'> 上传时间: <font color='blue'>"+this.createdDate+"</font></span>");
				html.push("<span class='upload-filename'> 删除时间: <font color='blue'>"+this.deleteDate+"</font></span>");
				html.push("<span class='upload-filename'> 上传步骤: <font color='blue'>"+this.uploadStep_+"</font></span>");
				html.push("<span class='upload-filename'> 删除步骤: <font color='blue'>"+this.deleteStep_+"</font></span>");
				html.push("<span class='upload-filename'> 删除人: <font color='blue'>"+this.deleteByName+"</font></span>");
				html.push("</li>");
				index ++;
			}
		}) ;
